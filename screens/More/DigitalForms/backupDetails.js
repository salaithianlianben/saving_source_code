...
class DigitalFormsDetails extends Component {
  ...
  _isCheckMultipleChoiceGrid = (rIndex, cIndex, index) => {
    ...
    tempFormSettingsData[index].value === undefined ? tempFormSettingsData[index].value = {row: rIndex, col: cIndex } : 
    tempFormSettingsData[index].value.row = rIndex ;
    tempFormSettingsData[index].value.col = cIndex ;
    ;

    console.log(tempFormSettingsData);
  ...
  _submitFormData = () => {
    ...
    if (isValid === true) {
      ...
      const options = {
        ...
        keyPrefix: configs.constant.S3_KEYS.AWS_KEY_PREFIX_FILE,
      ...
      //       console.log("---------------------------------------------------------")
      //       await RNS3.put(x.value, options)
      //         .then((response) => {
  ...
  multi = (v, i, i1) => {
    ...
    console.log(JSON.stringify(this.state.form_settings_data));
  };
    ...
    switch (item.type) {
      ...
      case 'Multiple choice':
        ...
        item.option.map((op, i) =>
          option.push({label: i + 1, id: i, value: op}),
        );
                  ...
                  {option.map((obj, i) => (
                    <RadioButton labelHorizontal={true} key={i}>
                      <RadioButtonInput
                        obj={obj}
                        index={i}
                        isSelected={item.value === i}
                        // onPress={onPress}
                        onPress={(value, index) =>
                          this.multi(value, index, Index1)
                        }
                        borderWidth={1}
                        buttonInnerColor={configs.colors.primaryColor}
                        buttonOuterColor={
                          item.value === i ? '#2196f3' : '#DADADA'
                        }
                        buttonSize={15}
                        buttonOuterSize={25}
                      />
            ...
            {item.value != null ? (
              <View
                ...
                {typeof item.value == 'string' ? (
                  <Text style={{color: configs.colors.primaryColor,flex:0.9,}}>
              ...
              <TouchableOpacity
                ...
                <Text>Upload</Text>
                <Space width={10} />
                <Entypo name="attachment" color={'grey'} size={12} />
              </TouchableOpacity>
            )}
            {this.state.is_submitted_for_validation === true &&
              item.mandatory &&
                              ...
                              <RadioButtonSN
                                ...
                                isSelected={
                                  ...
                                  item.value !== undefined
                                    ? item.value.row === tIndex &&
                                      item.value.col === rIndex - 1
                                    : false
                                  // false
                                }
                                onPress={() => 
        ...
        return (
          <View style={styles.item} key={index}>
            ...
            <View style={{marginLeft: 10}}>
              <AirbnbRating
                count={parseInt(item.option[1])}
                showRating={false}
                defaultRating={item.value != null ? parseInt(item.value) : 0}
                size={20}
                onFinishRating={(rating) => {
                  let tempData = this.state.form_settings_data;
                  tempData[index].value = rating;
                  this.setState({
                    form_settings_data: tempData,
                  });
                }}
    ...
    return (
      <SafeAreaView
        ...
        <StatusBar
          translucent={true}
          backgroundColor={
            this.state.isSuccess === true || this.state.is_expired === true || this.state.isPosting === true
              ? '#00000020'
              : 'white'
          }
        />
