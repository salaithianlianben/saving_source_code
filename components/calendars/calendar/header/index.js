...
class CalendarHeader extends Component {
  ...
  renderWeekDays = weekDaysNames => {
    ...
    return weekDaysNames.map((day, idx) => {
      ...
      return (
        <Text allowFontScaling={false} key={idx} style={[dayStyle,{ fontWeight:'700',fontSize:12}]} numberOfLines={1} accessibilityLabel={''}>
          {day}
        </Text>
      );
    });
  ...
  render() {
    const {style, testID,headerShown} = this.props;
